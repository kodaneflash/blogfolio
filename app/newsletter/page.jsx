// This should be the very first line in your file
"use client";

import React, { useEffect } from 'react';
import { motion } from 'framer-motion';
import { PiBellRingingLight } from 'react-icons/pi';
import { useAnimate, stagger } from 'framer-motion'; // Adjust this line based on actual imports
...
function Newspage() {
  ...
  }, [animate]); // Removed scope.current from dependencies

  const renderWords = () => {
    return (
      <motion.div ref={scope}>
        {wordsArray.map((word, index) => (
          <motion.span key={index} className="text-white opacity-0">
            {word}{" "}
          </motion.span>
        ))}
      </motion.div>
